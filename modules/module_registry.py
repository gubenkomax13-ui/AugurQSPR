...
MODULES = {
    ...
    "structural_audit": {
        ...
        "blocks": [
            ...
            },
            {
                ...
                "items": [
                    {
                        "name": "SAOD (Структурная проверка и контроль согласованности данных)",
    ...
    "advanced_validation": {
        ...
        "groups": [
            {
                "name": "Повторная кросс-валидация",
                "purpose": "Проверяет устойчивость оценки к разным случайным разбиениям.",
                "tools": [
                    {"name": "Repeated K-Fold", "purpose": "Повторяет K-Fold с разными разбиениями."},
                ],
            },
            {
                "name": "Переносимость на новую химию",
                "purpose": "Проверяет модель на отложенных структурах или заданных группах.",
                "tools": [
                    {"name": "Group / Scaffold split", "purpose": "Откладывает химические каркасы либо заданные пользователем группы."},
                ],
            },
            {
                "name": "Достаточность данных и интервалы",
                "purpose": "Проверяет достаточность данных и фактическое покрытие интервалов прогноза.",
                "tools": [
                    {"name": "Learning curves", "purpose": "Отделяют дефицит данных от переобучения."},
                    {"name": "Покрытие hold-out prediction interval", "purpose": "Сопоставляет фактическое покрытие hold-out с заданным уровнем интервала."},
                ],
            },
        ],
        "difference": "Основная валидация оценивает качество одной стандартной проверкой; расширенная проверяет устойчивость этой оценки, переносимость на новые структуры и достаточность данных.",
    },
    "diagnostics": {
